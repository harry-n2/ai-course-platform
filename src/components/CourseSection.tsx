      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="mb-16">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div className="px-6 py-2 bg-slate-900 text-white text-sm font-bold tracking-wider">
              FREE — 無料プラン（Day 1〜10）
            </div>
            <div className="h-px flex-1 bg-slate-200" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {freeDays.map((item) => (
              <div
                key={item.day}
                className={`p-5 border ${item.highlight ? "border-[#d4af37] bg-[#d4af37]/5" : "border-slate-200"}`}
              >
        ...
        <div className="mb-12">
          <div className="flex items-center gap-4 mb-8">
            <div className="h-px flex-1 bg-slate-200" />
            <div className="px-6 py-2 bg-[#d4af37] text-slate-900 text-sm font-bold tracking-wider">
          ...
          <div className="space-y-3">
            {paidModules.map((item) => (
              <div key={item.module} className="flex items-start gap-4 p-5 border border-slate-200 hover:border-[#d4af37] transition-colors group">
        ...
        {/* CTA */}
        <div id="course-register" className="bg-slate-900 p-10 text-center">
          <h3 className="text-2xl font-black text-white mb-3">まずは無料で体験する</h3>
          <p className="text-slate-400 mb-8">Day 1〜10を無料で受講。AI活用の基礎を身につけてから、次のステップを判断できます。</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/register"
              className="inline-flex items-center justify-center px-10 py-4 bg-white text-slate-900 font-bold text-sm tracking-wider hover:bg-[#d4af37] hover:text-white transition-colors"
            >
            ...
            <Link
              href="/checkout"
              className="inline-flex items-center justify-center px-10 py-4 border-2 border-[#d4af37] text-[#d4af37] font-bold text-sm tracking-wider hover:bg-[#d4af37] hover:text-slate-900 transition-colors"
            >
              有料プランを申し込む
            </Link>
          </div>
