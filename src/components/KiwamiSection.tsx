...
export function KiwamiSection() {
  return (
    <section id="kiwami" className="py-16 md:py-24 bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="text-center mb-10 md:mb-16">
          ...
          <p className="text-slate-600 text-base md:text-lg max-w-2xl mx-auto">
            人生を変える「具体的手段」を、約7日で。<br className="hidden sm:block" />
            あなたの代わりに、集客・教育・販売するシステムを丸ごと構築します。
          </p>
        </div>

        {/* Pain Points */}
        <div className="bg-slate-900 p-6 md:p-8 mb-10 md:mb-16 max-w-3xl mx-auto">
          <h3 className="text-white font-bold text-base md:text-lg mb-5 md:mb-6 text-center">
            こんな課題を抱えていませんか？
          </h3>
          <div className="space-y-3 md:space-y-4">
            {painPoints.map((pain, i) => (
              <div key={i} className="flex items-start gap-3">
                <span className="text-[#d4af37] font-black text-lg leading-none mt-0.5 shrink-0">×</span>
                <p className="text-slate-300 text-sm md:text-base">{pain}</p>
              </div>
            ))}
          </div>
          <div className="mt-6 md:mt-8 pt-5 md:pt-6 border-t border-slate-700 text-center">
        ...
        <div className="mb-10 md:mb-16">
          <h3 className="text-center text-xl md:text-2xl font-black text-slate-900 mb-3">
            パッケージ「極」の全貌
          </h3>
          <p className="text-center text-slate-500 text-sm mb-8 md:mb-10">
            通常1〜2ヶ月かかる構築を、AI爆速納品で約7日に圧縮。毎月<strong>2名様限定</strong>。
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {features.map((f) => (
              <div key={f.title} className="bg-white p-5 md:p-6 border border-slate-200 hover:border-[#d4af37] transition-colors group">
                <f.icon size={24} className="text-[#d4af37] mb-3 md:mb-4 group-hover:scale-110 transition-transform" />
                <h4 className="font-bold text-slate-900 mb-2">{f.title}</h4>
                <p className="text-sm text-slate-500 leading-relaxed">{f.desc}</p>
              </div>
            ))}
            <div className="bg-[#d4af37] p-5 md:p-6 flex flex-col justify-center">
        ...
        {/* Role division */}
        <div className="bg-white border border-slate-200 p-6 md:p-8 mb-10 md:mb-16 max-w-3xl mx-auto">
          <h3 className="text-center text-lg md:text-xl font-black text-slate-900 mb-6 md:mb-8">役割分担</h3>
        ...
        <div className="text-center">
          ...
          <Link
            ...
            className="inline-flex items-center justify-center w-full sm:w-auto px-10 md:px-12 py-4 md:py-5 bg-[#d4af37] text-slate-900 font-black text-sm tracking-wider hover:bg-[#c09b28] transition-colors"
          >
            無料相談を予約する →
          </Link>
        </div>
